...
import uuid
import copy
from CausalEEGNet import CausalEEGNet
import random
...
def compute_contrastive_loss_zu(zu, domain_ids, temperature=0.1):
    ...
    # 如果某个样本没有正样本 (该 batch 中只有一个来自该被试)，跳过
    pos_count = pos_mask.sum(dim=1)  # 每个样本的正样本数量
    ...
    # log(exp(sim_pos) / denominator) = sim_pos - log(denominator)
...
DEFAULT_PREPROCESS_CFG = {
    ...
    'bandpass': (4, 40),
    'filter_padding': 100,
    'enable_memmap': False,  # 禁用memmap以避免磁盘配额问题
    'memmap_threshold_mb': 1024,
    'memmap_cache_dir': '/tmp/eeg_memmap_cache',  # 使用临时目录
}
...
def unpack_causal_outputs(outputs):
    """
    兼容不同版本 CausalEEGNet 的输出，确保训练流程始终可运行。
    """
...
def load_folder_filtered(folder_path, collect_domains=False, subject_to_idx=None, preprocess_cfg=None):
    ...
    for subj in subject_dirs:
        ...
        for root, dirs, _ in os.walk(subj_path):
            ...
            has_three = '3class' in name_lower
            if has_three or has_offline:
                mat_files.extend(glob.glob(os.path.join(root, "*.mat")))

    if not mat_files:
        print("   [WARN] 未找到 OfflineImagery + 3class 数据，尝试直接扫描根目录 .mat ...")
...
def train_causal_model(X_train_full, Y_train_full, domains_train_full,
                       ...
                       domains_test=None):
    print("=" * 60)
    print(f'[INFO] 启动因果驱动训练 (Causal-Driven Training)')
    ...
    log_dir = os.path.join('runs', f'Causal_Random_{run_id}')
    ...
    # --- 🔥 [关键修复] 为解决 nn.DataParallel 无法处理 LazyLinear 的问题 ---
    # LazyLinear 需要第一次 forward 后才初始化参数，而 DataParallel 要求初始化后才能复制模型。
    print("[INFO] 正在进行一次 Dummy Forward 以初始化 LazyLayers...")
    model.eval() # 临时设为 eval 模式以跳过 BatchNorm 的样本数检查
    with torch.no_grad():
        dummy_input = torch.randn(2, 1, n_chans, n_samples).to(device) # batch_size=2 增加统计鲁棒性
        _ = model(dummy_input)
    model.train() # 恢复为训练模式
    
    # 【关键】多显卡并行逻辑
    if torch.cuda.device_count() > 1:
        print(f"[INFO] 检测到 {torch.cuda.device_count()} 块 GPU，已开启 nn.DataParallel")
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            # VIB: KL 到标准正态（有重参数化时使用 mu/logvar，兼容旧版）
            ...
            # 临时调试打印（Causal 阶段首个 batch）
            if epoch == 21 and i == 0:
                print(f"\n[DEBUG] Raw HSIC: {loss_hsic.item()}")
                print(f"[DEBUG] Raw REC: {loss_rec.item()}")
                print(f"[DEBUG] Input scale: {inputs.abs().mean().item()}")
            ...
            # 🔥 Zu 对比学习损失: 让同被试的 Zu 聚集，不同被试的 Zu 远离
            ...
            # --- EMA 更新逻辑 (适配 LazyLinear + BatchNorm 统计 + DataParallel) ---
            # 如果使用了 DataParallel，需要通过 model.module 获取原始参数
            actual_model = model.module if isinstance(model, nn.DataParallel) else model
            
            if ema_model is None:
                ema_model = copy.deepcopy(actual_model)
                ema_model.to(device)
                ema_model.eval()
            else:
                with torch.no_grad():
                    for param_ema, param_net in zip(ema_model.parameters(), actual_model.parameters()):
                        param_ema.data.mul_(ema_decay).add_(param_net.data, alpha=1 - ema_decay)
                    for buffer_ema, buffer_net in zip(ema_model.buffers(), actual_model.buffers()):
                        if buffer_ema.is_floating_point():
                            buffer_ema.data.mul_(ema_decay).add_(buffer_net.data, alpha=1 - ema_decay)
                        else:
                            buffer_ema.data.copy_(buffer_net.data)
            ...
            running_losses['vib'] += loss_vib.item() if isinstance(loss_vib, torch.Tensor) else loss_vib
        ...
        log_str = (f"Epoch {epoch+1:03d} [{phase}] | "
                   ...
                   f"Z:{running_losses['zu_adv']/n_batches:.2f}, "
                   f"V:{running_losses['vib']/n_batches:.4f}, "
        ...
        writer.add_scalar('Loss/Class', running_losses['cls']/n_batches, epoch)
        ...
        writer.add_scalar('Loss/ZuAdv', running_losses['zu_adv']/n_batches, epoch)
        writer.add_scalar('Loss/VIB', running_losses['vib']/n_batches, epoch)
        writer.add_scalar('Loss/Contrastive', running_losses['contrastive']/n_batches, epoch)
        writer.add_scalar('Metric/ValAcc', val_acc, epoch)
        writer.add_scalar('Param/Alpha', current_alpha, epoch)
        writer.add_scalar('Param/LambdaDomain', current_lambda_domain, epoch)

        scheduler.step(val_acc)

        if val_acc > best_acc:
            best_acc = val_acc
            # 保存泛化性更好的 EMA 模型 (如果尚未初始化则保存原模型)
            save_obj = ema_model.state_dict() if ema_model is not None else model.state_dict()
    ...
    if test_loader is not None:
        ...
        # 兼容并行模型的加载（确保加载到 model.module 或者原始 model）
        actual_model = model.module if isinstance(model, nn.DataParallel) else model
        actual_model.load_state_dict(torch.load(save_name, map_location=device))
        actual_model.eval()
        correct = 0
        total = 0
        ...
        print(f"[FINAL RESULT] CausalEEGNet Test Accuracy: {test_acc:.4f}")
    ...
    actual_model_tsne = model.module if isinstance(model, nn.DataParallel) else model
    actual_model_tsne.load_state_dict(torch.load(save_name, map_location=device))
    actual_model_tsne.eval()
    
    # 用完整训练集（包含 domains）做推理
...
if __name__ == "__main__":
    ...
    params = {
        ...
        'lambda_zu_adv': 0.1,
        'gamma_vib': 0.01,
        'lambda_contrastive': 0.1,  # 🔥 Zu对比学习权重
        'ema_decay': 0.99,
        'label_smoothing': 0.1,
        'augment': {
            'coverage': 0.0,
    ...
    print(f"[TRAIN] 加载训练集 (仅 OfflineImagery & 3class)...")
    ...
    print(f"[EVAL] 加载测试集 (仅 OfflineImagery & 3class)...")
    ...
    save_path = os.path.join(save_folder, 'Causal_TrainTestSplit_3Class.pth')
    
    # 随机划分无需指定 val_domain_id
